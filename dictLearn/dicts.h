...
template <typename T> class Trainer {
   public:
	  ...
	  void getBasis(Matrix<T>& basis) const { basis.copy(_basis); };
	  void getSpCoef(Matrix<T>& spCoef) const { spCoef.copy(_spCoef); };
...
void Trainer<T>::cleanDict(const Data<T>& X, Matrix<T>& G, 
      ...
      const T gamma2,
      const T maxCorrel) {
   int sparseD = modeD == L1L2 ? 2 : 6;			//modeD = L2, sparseD = 6
   const int k = _D.n();			// k = 256
   const int n = _D.m();			// n = 64
   const int M = X.n();				// M = 255025
   T* const pr_G=G.rawX();			// pr_G -> G
   Vector<T> aleat(n);
   Vector<T> col(n);
   for (int i = 0; i<k; ++i) {
      //pr_G[i*k+i] += 1e-10;
      for (int j = i; j<k; ++j) {
         if ((j > i && abs(pr_G[i*k+j])/sqrt(pr_G[i*k+i]*pr_G[j*k+j]) > maxCorrel) ||	// for non-diagonal, abs( G(i,j) / sqrt(G(i,i)*G(j,j) ) ) > maxCorrel 
               (j == i && abs(pr_G[i*k+j]) < 1e-4)) {			// for dignonal, abs(G(i,j) < 1e-4 )
            /// remove element j and replace it by a random element of X
            const int ind = random() % M;			// 
            Vector<T> d, g;
            _D.refCol(j,d);			
            X.getData(col,ind);     
            d.copy(col);			// D[j] = X[ind]
            if (modeD != L2) {
               aleat.copy(d);
...
void Trainer<T>::train(const Data<T>& X, const ParamDictLearn<T>& param) {
   ...
   T rho = param.rho;
   T t0 = param.t0;
   int sparseD = param.modeD == L1L2 ? 2 : param.modeD == L1L2MU ? 7 : 6;			// param.modeD = 0, ||d_j||_2^2 <= 1 }
   ...
   const int batchsize= param.batch ? M : MIN(_batchsize,M);		// batchsize = 50
   ...
   srandom(0);
   Vector<T> col(n);			// col._n = n(64)
   if (!_initialDict) {
      _D.resize(n,K);			// D - 64 * 256
      for (i = 0; i<K; ++i) {	// Initialize Dictionary - randomly choose columns from Data
         const int ind = random() % M;
         Vector<T> d;			
         _D.refCol(i,d);		// d = &D[i]
         X.getData(col,ind);	// col = X[col]
         d.copy(col);			// d = col
      }
      _initialDict=true;
   }
   ...
   if (param.modeD != L2) {
      ...
      _D.normalize();		// Normalize columns of D
   }

   int count=0;
   int countPrev=0;
   T scalt0 =  abs<T>(t0);		// scalt0 = t0 = 1e-5
   if (_itercount == 0) {
      _A.resize(K,K);			// A - 512 * 512
      _A.setZeros();			
      _B.resize(n,K);			// B - 102 * 512
      _B.setZeros();
      if (!param.batch) {
         _A.setDiag(scalt0);	// set the diagonal of A using scalt0
         _B.copy(_D);			// B = D
         _B.scal(scalt0);		// B *= scalt0
      }
   }

   //Matrix<T> G(K,K);
   // added by Kai
   _basis.resize(n, K*param.iter,1);
   _spCoef.resize(K, param.iter*batchsize,1);
   ...
   for (i = 0; i<NUM_THREADS; i++) {
      spcoeffT[i].resize(K);		// spcoeffT - 256
      DtRT[i].resize(K);			// DtRT - 256
      XT[i].resize(n);				// XT - 64
      BT[i].resize(n,K);			// BT - 64 * 256
      BT[i].setZeros();				
      AT[i].resize(K,K);			// AT - 256 * 256
      AT[i].setZeros();
      GsT[i].resize(L,L);			// GsT - 64 * 64
      GsT[i].setZeros();
      invGsT[i].resize(L,L);		// invGsT - 64 * 64
      invGsT[i].setZeros();
      GaT[i].resize(K,L);			// GaT - 256 * 64
      GaT[i].setZeros();
      workT[i].resize(K,3);			// workT - 256 * 3
      workT[i].setZeros();
      uT[i].resize(L);				// uT - 64
      uT[i].setZeros();
   }
   ...
   srandom(0);
   Vector<int> perm;
   perm.randperm(M);				// perm - permutation between 0 to 255024 duplication ? 

   Aodd.setZeros();					
   Bodd.setZeros();
   Aeven.setZeros();
   Beven.setZeros();
   Aorig.copy(_A);					// Aorig - 256 * 256
   Borig.copy(_B);					// Borig - 64 * 256

   int JJ = param.iter < 0 ? 100000000 : param.iter;		// number of Iterations
   bool even=true;
   int last_written=-40;
   
   for (i = 0; i<JJ; i++) {			// top iteration for sparse coding and dictionary learning
      if (param.verbose) {
      ...
      G.addDiag(MAX(param.lambda2,1e-10));			// G[diag] += MAX(param.lambda2,1e-10)
      int j;
...
#pragma omp parallel for private(j)
      for (j = 0; j<batchsize; j++) {
         ...
         int numT=0;
#endif
         const int index=perm[(j+i*batchsize) % M];		// index = perm(i,j)
         Vector<T>& Xj = XT[numT];						// Xj -> XT[0]
         SpVector<T>& spcoeffj = spcoeffT[numT];		// spcoeffj -> spcoeffT[0]
         Vector<T>& DtRj = DtRT[numT];					// DtRj -> DtRT[0]
         //X.refCol(index,Xj);
         X.getData(Xj,index);							// Xj = X[index]
         if (param.whiten) {
            if (param.pattern) {
               Vector<T> mean(4);
         ...
         _D.multTrans(Xj,DtRj);				// DtRj(256) = D(64 * 256)' * Xj(64)
         Matrix<T>& Gs = GsT[numT];			// Gs - 64 * 64	
         Matrix<T>& Ga = GaT[numT];			// Ga - 256 * 64
         Matrix<T>& invGs = invGsT[numT];	// invGs - 64 * 64
         Matrix<T>& work= workT[numT];		// work - 256 * 3
         Vector<T>& u = uT[numT];			// u - 64
         Vector<INTM> ind;
         Vector<T> coeffs_sparse;
         spcoeffj.setL(L);					// Length = 64
         spcoeffj.refIndices(ind);			// ind -> spcoeffj._r(indice)
         spcoeffj.refVal(coeffs_sparse);	// coeffs_sparse -> spcoeffj._v(data)
         T normX=Xj.nrm2sq();				// normX = sqrt(||Xj||_2)
         coeffs_sparse.setZeros();			// clear coeffs_sparse, spcoeffj
         if (param.mode < SPARSITY) {		// solve min_{D in C} (1/n) sum_{i=1}^n (1/2)||x_i-D*alpha_i||_2^2 + lambda||alpha_i||_1 + lambda_2||alpha_i||_2^2
            if (param.mode == PENALTY && param.lambda==0 && param.lambda2 > 0 && !param.posAlpha) {
               Matrix<T>& GG = G;
               u.set(0);
			   ...
			   // added by Kai
			   _spCoef.setCol(j + i*batchsize, coeffs_sparse, ind);
         ...
         int count2=0;
         for (int k = 0; k<L; ++k)			// count number of non-zeros elements in coeffs_sparse
            if (ind[k] == -1) {
               break;
            } else {
               ++count2;
         ...
         spcoeffj.setL(count2);					// spcoeffj._L = count2
         AT[numT].rank1Update(spcoeffj);		// AT += spcoeffj' * spcoeffj i.e. A(t) = A(t-1) + alpha_t * alpha_t'
         BT[numT].rank1Update(Xj,spcoeffj);		// BT += Xj' * spcoeffj i.e. B(t) = B(t-1) + X(t) * alpha_t
      }

      if (param.batch) {						// use batch learning instead of online learning
         _A.setZeros();
         _B.setZeros();
         for (j = 0; j<NUM_THREADS; ++j) {
            _A.add(AT[j]);
      ...
      } else if (param.stochastic) {			// use stochastic learning
         _A.setZeros();
         _B.setZeros();
         for (j = 0; j<NUM_THREADS; ++j) {
            _A.add(AT[j]);
      ...
      } else {			// use online dictionary learning
         ...
         T scal2=T(T(1.0)/batchsize);		// scal2 = 0.0025
         T scal;
         ...
         } else {			// choose parameter free strategy used in ICML paper
            scal = T(_itercount+num_elem+1-
				batchsize) / T(_itercount + num_elem + 1);			// scal = (_itercount+num_elem+1-batchsize) / (_itercount + num_elem + 1)
         }
         Aeven.scal(scal);		// Aeven *= scal
         Beven.scal(scal);		// Beven *= scal
         Aodd.scal(scal);		// Aodd *= scal
         Bodd.scal(scal);		// Bodd *= scal
         if ((_itercount > 0 && i*batchsize < M) 
               || (_itercount == 0 && t0 != 0 && 
                  i*batchsize < 10000)) {
            Aorig.scal(scal);		// Aorig *= scal
            Borig.scal(scal);		// Borig *= scal
            _A.copy(Aorig);			// A = Aorig
            _B.copy(Borig);			// B = Borig
         } else {
            _A.setZeros();
            _B.setZeros();
         }
         for (j = 0; j<NUM_THREADS; ++j) {
            Aeven.add(AT[j],scal2);			// Aeven += AT * scal2
            Beven.add(BT[j],scal2);			// Beven += BT * scal2
         }
         _A.add(Aodd);			
         _A.add(Aeven);			// A = Aorig + Aodd + Aeven
         _B.add(Bodd);			
         _B.add(Beven);			// B = Borig + Bodd + Beven
         ++count;
		 ++countPrev;
         ...
         Vector<T> newd(n);
		 newd.setZeros();
         for (j = 0; j<param.iter_updateD; ++j) {			// number of iterations for dictionary update, default = 1 
            for (int k = 0; k<K; ++k) {		// update one column each iteration
               //if (_A[k*K+k] > 1e-6) {		// A[diagonal] > 1e-6, in case A(k,k) = 0
				if (abs(_A[k*K + k]) > 1e-8) {		// A[diagonal] > 1e-6, in case A(k,k) = 0
                  _D.refCol(k,di);			// di -> D[k]
                  _A.refCol(k,ai);			// ai -> A[k]
                  _B.refCol(k,bi);			// bi -> B[k]
                  _D.mult(ai,newd,T(-1.0));	// newd = -_D * ai
                  newd.add(bi);				// newd = bi - D * ai
                  newd.scal(T(1.0)/_A[k*K+k]);			// newd = (bi - D * ai) / A(k,k)
                  newd.add(di);				// newd = (bi - D * ai) / A(k,k) + di

				  if (newd.isAllZeros() == 1) {
					  int a = 1;
				  }
                  ...
                  if (param.modeD != L2) {
                     ...
                     if (param.posD) newd.thrsPos();		// positive constrained Dictionary
                     newd.normalize2();		// Normalize newd
                     di.copy(newd);			// di = newd
                  }
				  // added by Kai
				  _basis.refCol(K * i + k, ddup);
				  ddup.copy(newd);
