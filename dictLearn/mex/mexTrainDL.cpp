...
template <typename T>
   inline void callFunction(mxArray* plhs[], const mxArray*prhs[],
      ...
      Data<T> *X;
      const mwSize* dimsX=mxGetDimensions(prhs[0]);		// dixmsX[0] - rows of objection X, dixmsX[1] - cols of objection X
      INTM n=static_cast<INTM>(dimsX[0]);				// signal dimension - 102
      INTM M=static_cast<int>(dimsX[1]);				// epochs - 3161 
      if (mxIsSparse(prhs[0])) {
         ...
         T* prX = reinterpret_cast<T*>(mxGetPr(prhs[0]));			// prX -> X
         X= new Matrix<T>(prX,n,M);			// reinterpret data into X
      }
      ...
      NUM_THREADS=1;
#endif 
      int batch_size = getScalarStructDef<int>(prhs[1],"batchsize",			// batch_size = 50
            256*(NUM_THREADS+1));
      mxArray* pr_D = mxGetField(prhs[1],0,"D");			// pr_D -> D
      Trainer<T>* trainer;

      if (!pr_D) {			// D is allocated yet
         int K = getScalarStruct<int>(prhs[1],"K");			// K = number of atoms
      ...
      param.iter_updateD = getScalarStructDef<T>(prhs[1],"iter_updateD",param.batch ? 5 : 1);			// iterate once on updating D 
      ...
      trainer->train(*X,param);			// start training the dictionary
      if (param.log)
         mxFree(param.logName);

      Matrix<T> D;
      trainer->getD(D);					// get dictionary
      int K  = D.n();				
      plhs[0] = createMatrix<T>(n,K);	// create matrix D
      T* prD2 = reinterpret_cast<T*>(mxGetPr(plhs[0]));			// prD2(double *) = plhs[0](mxArray *) -> D
      Matrix<T> D2(prD2,n,K);
      D2.copy(D);

	  Matrix<T> basis;
	  trainer->getBasis(basis);
	  plhs[1] = createMatrix<T>(n,K*param.iter);
	  T* prBasis = reinterpret_cast<T*>(mxGetPr(plhs[1]));
	  Matrix<T> Basis2(prBasis, n, K*param.iter);
	  Basis2.copy(basis);

	  Matrix<T> matSpCoef;
	  trainer->getSpCoef(matSpCoef);
	  plhs[2] = createMatrix<T>(K, param.iter*batch_size);
	  T* prSpCoef = reinterpret_cast<T*>(mxGetPr(plhs[2]));
	  Matrix<T> matSpCoef2(prSpCoef, K, param.iter*batch_size);
	  matSpCoef2.copy(matSpCoef);
