...
rsnr_dl = zeros(mdivision,1);
res_dl = zeros(mdivision,1);
sparsity_dl = zeros(mdivision,1);
prd_dl = zeros(mdivision,1);
...
filename = sprintf('./Results/m%d_haar_basis_1e-6.mat', mdivision);
save(filename)
...
subplot(3,1,1)
for i = 1 : mdivision-1
    plot(floor(j * 50),repmat(rsnr_dl(i),1,length(j)),'Color',cc(i,:) ) ;
...
subplot(3,1,2)
for i = 1 : mdivision-1
    plot(floor(j * 50),repmat(prd_dl(i),1,length(j)),'Color',cc(i,:) ) ;
    str{i}=['m=',num2str(floor(i * n_dl / mdivision))];
    hold on
end
% legend(str)
...
subplot(3,1,3)
for i = 1 : mdivision-1
    plot(floor(j * 50),repmat(sparsity_dl(i),1,length(j)),'Color',cc(i,:) ) ;
    str{i}=['m=',num2str(floor(i * n_dl / mdivision))];
    hold on
end
% legend(str)
