...
template<typename T> class Matrix : public Data<T>, public AbstractMatrix<T>, public AbstractMatrixB<T> {
   ...
   // added by Kai
   inline void setCol(const INTM col, Vector<T>& coe, Vector<INTM>& ind);
...
// added by Kai
template <typename T> inline void Matrix<T>::setCol(const INTM col, Vector<T>& coe, Vector<INTM>& ind) {
	T* coeX = coe.rawX();
	INTM* indX = ind.rawX();

	for (INTM i = 0; i < ind.n(); i++) {
		INTM index = col*_m + indX[i];
		T data = coeX[i];
		if (data != 0)
			_X[index] = data;
	}
...
// added by Kai
template <typename T> inline bool Vector<T>::isAllZeros() {
	double sum = 0.0;
	for (INTM i = 0; i < _n; i++) {
		sum += abs(_X[i]);
	}
