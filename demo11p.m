...
% Pre-processing data through DCT
% % % % % % % % % % % % % % % % % % % % % % % % % % %
...
% subplot(2,1,2)
% plot(TrainInpDCT(:,1));
spar = 1- length(find(abs(TrainInpDCT)>15) ) / size(TrainInpDCT,1) / size(TrainInpDCT,2);
...
samplesTrainDCT = size(TrainInpDCT,2);
samplesTest  = size(TestInp,2);

rsnr_dl = zeros(mdivision,length(1:floor(samplesTrainDCT / 50)));
cr_dl = zeros(mdivision,length(1:floor(samplesTrainDCT / 50)));
prd_dl = zeros(mdivision,length(1:floor(samplesTrainDCT / 50)));
sparsity_dl = zeros(mdivision,length(1:floor(samplesTrainDCT / 50)));
basis = cell(mdivision,length(1:floor(samplesTrainDCT / 50)));
R1 = cell(mdivision,length(1:floor(samplesTrainDCT / 50)));
alpha = cell(mdivision,length(1:floor(samplesTrainDCT / 50)));
% R2 = cell(mdivision,length(1:floor(samplesTrainDCT / 50)));
reconSig = cell(mdivision,length(1:floor(samplesTrainDCT / 50)));
...
%%
 ...
 for i = 1:mdivision 
    ...
    for j = 1 : floor(samplesTrainDCT / 50)      % adjust iter
        param = struct;
        param.iter = j;
        param.batchsize = 50;
        param.K = 512;
...
filename = sprintf('./Results/PreDCT_m%d_batchsize%d_lambda%.2f.mat', mdivision, 50, 0.10);
save(filename)
...
% 
% figure
% epV = 50 * (1 : floor(samplesTrainDCT / 50)); 
