% % % % % % % % % % % % % % % % % % % % % % % % % % %
% Test sparsity based upon dictionary
% % % % % % % % % % % % % % % % % % % % % % % % % % %
...
mu = zeros(1, size(phi_dl,1) );

for i = 1:size(phi_dl,1)
    for j = 1:size(psi_dl,2)
        mu(i) = max(phi_dl(i,:) .* psi_dl(:,j)' );
    end
end
...
figure
plot(floor(0.95*n_dl: 0.01*n_dl: n_dl)./n_dl,sparsity_dl);
xlabel('m/n, n = 102');
ylabel('L0-norm of coefficients');


save './Results/RSNRvsSparsity.mat'